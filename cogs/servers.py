...
class ServerCommands(commands.Cog):
    ...
    async def findgametype(
        ...
        try:
            ...
            s_gametype = sanitize_text(gametype)
            embed = discord.Embed(title=f"Servers Playing: {s_gametype}", color=discord.Color.orange())
            for server in server_list:
                players = f"{server['current_player_count']}/{server['current_max_players']}"
    ...
    async def serverinfo(
        ...
        try:
            ...
            # Data Extraction (with sanitization)
            hostname = sanitize_text(server['current_server_name'])
            map_name = sanitize_for_codeblock(server['current_map'] or 'N/A')
            num_players = server['current_player_count']
            max_players = server['current_max_players']
            game_mod = sanitize_for_codeblock(server['unpure_mods'] or server['current_gametype'] or 'N/A')
            gametype = sanitize_for_codeblock(server['current_gametype'] or 'N/A')
