        ...
        try:
            ...
            if not map_rows:
                ...
                for i, row in enumerate(map_rows):
                    s_map_name = sanitize_text(row['map_name'])
                    map_desc += f"{i+1}. **{s_map_name}** ({row['count']} subs)\n"
            ...
            if not server_rows:
                ...
                for i, row in enumerate(server_rows):
                    s_server_name = sanitize_text(row['server_name'])
                    server_desc += f"{i+1}. **{s_server_name}** ({row['count']} subs)\n"
    ...
    async def find(
        ...
        try:
            ...
            if found_player:
                ...
                embed = discord.Embed(
                    title=f"🕵️ Player Found: {s_player_name}",
                    color=discord.Color.blue()
                )
                s_server_name = sanitize_text(found_player['current_server_name'])
                embed.add_field(name="Server", value=s_server_name, inline=False)
