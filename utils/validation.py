...
def sanitize_for_codeblock(text: str) -> str:
    """
    Escapes backticks for use inside a Discord code block.
    Discord doesn't have a standard way to escape backticks INSIDE a code block,
    so we replace triple backticks with a similar-looking character to prevent breakout.
    """
    if not text:
        return ""
